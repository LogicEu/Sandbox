...
Entity entity_roulette(texture_t t)
{
    ...
    unsigned int x = rand() % (int)(width / scale);
    unsigned int y = rand() % (int)(height / scale);
    ...
    for (int i = 0; i < entity_count(); i++) {
        if (entity_has(i, COMP_POS)) {
            ...
            if (entity_has(i, COMP_VEL)) {
                ...
                if (pos->x > width / scale || pos->x < 0.0f) vel->x *= -1.0f;
                if (pos->y > height / scale || pos->y < 0.0f) vel->y *= -1.0f;
                *pos = vec2_add(*pos, vec2_mult(*vel, delta_time));
            }
            if (entity_has(i, COMP_TEX)) {
                if (entity_has(i, COMP_COLOR)) shader_set_uniform(shader, 4, "color", (float*)entity_get(i, COMP_COLOR));
                else shader_set_uniform(shader, 4, "color", &c[0]);
...
int main(int argc, char** argv)
{
    ...
    screen_color(background[0], background[1], background[2], background[3]);
